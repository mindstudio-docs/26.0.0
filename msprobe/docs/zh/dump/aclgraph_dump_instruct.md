...
| 参数名 | 可选/必选 | 说明 |
| --- | --- | --- |
| x | 必选 | 待保存的张量，torch.Tensor类型。 |
| path | 必选 | 保存路径（支持相对/绝对路径），str类型。实际落盘文件名会在该路径的文件名基础上追加序号，格式为 `{base}_{seq}.pt`。例如传入 `./dump/act.pt`，实际落盘为 `./dump/act_0.pt`、`./dump/act_1.pt`（其中父目录`./dump/`须为已存在的目录文件夹）。|
