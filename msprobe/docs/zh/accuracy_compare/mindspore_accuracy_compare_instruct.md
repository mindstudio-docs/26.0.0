...
| -o或--output_path  | 可选     | 配置比对结果文件存盘目录，默认会在当前目录创建output目录，str类型。文件名称基于时间戳自动生成，格式为：<br>&#8226; `compare_result_{timestamp}.xlsx`<br/>&#8226; `compare_result_{rank_id}_{step_id}_{timestamp}.xlsx`（仅[不同版本下的全量kernel比对](#不同版本下的全量kernel比对)场景支持）。<br>提示：output目录下与结果文件的同名文件将被删除覆盖。 |
